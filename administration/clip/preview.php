<?php
	...
	if( isset( $_REQUEST['clip_id'] ) && $_REQUEST['clip_id'] != "" ){
	$clipID = $_REQUEST['clip_id'];
	
	$conn = getConnection();
	$clips = new Clip($conn);
	$clip = $clips->load( $clipID );
	...
	if( isset( $_REQUEST['type'] ) &&  $_REQUEST['type'] != ""){
		//try and grab the clip path
		$type = $_REQUEST['type'];
		$path = $clips->getVidPath($clipID, $type);
	}else{
		//default to original
		$path = $clips->getVidPath($clipID, 1);
